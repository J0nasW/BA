...
def I_syn_calc(u_pre, u_post, E, w, G_syn, V_shift, V_range):

    I_s = w * (G_syn/1 + np.exp(-(u_pre - V_shift)/V_range)) * (E - u_post)

    return I_s

def I_gap_calc(u_pre, u_post, w_gap, G_gap):

    I_g = w_gap * G_gap * (u_pre - u_post)
