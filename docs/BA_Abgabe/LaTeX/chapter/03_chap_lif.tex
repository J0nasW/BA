		...
		\For{i $\leftarrow$ 1 \textbf{to} 4}{
			\For{j $\leftarrow$ 1 \textbf{to} 4}{
				\uIf{$\boldsymbol{A_{i,j}} == 1$}{
					$I_{inter}{i,j}$ = I\_syn\_calc($x[i], x[j], E_{in}, w[k], \sigma[k], \mu$)\\
					k $\leftarrow$ k + 1
				}
				\uElseIf{$\boldsymbol{A_{i,j}} == 2$}{
					$I_{inter}{i,j}$ = I\_syn\_calc($x[i], x[j], E_{ex}, w[k], \sigma[k], \mu$)\\
					k $\leftarrow$ k + 1
				}
				\uElse{}{
					$I_{inter}{i,j}$ = 0
				...
				$I_{inter}{i,j}$ = I\_syn\_calc($x[i], x[j], E, w[k], \sigma[k], \mu$)\\
				$I_{sensor}{i,j}$ = I\_syn\_calc($u[i], u[j], E, w[k], \sigma[k], \mu$)\\
				k $\leftarrow$ k + 1\\
				$I_{gap}{i,j}$ = I\_gap\_calc($x[i], x[j], \hat{w[l]}$)\\
				k $\leftarrow$ l + 1\\
			}
		}
